    ...
    public class PlayerHandler : SingeSend, IOpHandler
    {
        ...
        private void onToClientAdd(MobaClient client, bool result, int requestId)
        {
            ...
            if (result == true)
            {
                ...
                playerCache.AddFrient(playerId, requestId);
                Send(client, OpCode.PlayerCode, OpPlayer.ToClientAdd, 1,
                    "添加成功", JsonMapper.ToJson(toDto(playerCache.GetModel(playerId))));
                Send(requestClient, OpCode.PlayerCode, OpPlayer.ToClientAdd, 1,
                    "添加成功", JsonMapper.ToJson(toDto(playerCache.GetModel(requestId))));
                return;
            }
        ...
        private void onAdd(MobaClient client, string addName)
        {
            ...
            if (toModel == null)
            {
                Send(client, OpCode.PlayerCode, OpPlayer.RequsetAdd, -1, "没有此角色");
                return;
            }
            //如果添加的是自身那么不可以
            if (playerCache.GetModel(client).Id == toModel.Id)
            {
                Send(client, OpCode.PlayerCode, OpPlayer.RequsetAdd, -3, "不能添加自身");
                return;
            }
            //如果已经是好友的玩家 那么也不可以
            ...
            foreach (var item in friends)
            {
                if (string.IsNullOrEmpty(item))
                    continue;
                if (int.Parse(item) == toModel.Id)// "2asd"
                {
                    Send(client, OpCode.PlayerCode, OpPlayer.RequsetAdd, -4, "该玩家已经是好友");
                    return;
                }
            }
            //如果能获取到数据模型 先判断他是否在线
            ...
            if (!isOnline)
            {
                Send(client, OpCode.PlayerCode, OpPlayer.RequsetAdd, -2, "此玩家不在线");
                return;
            }
        ...
        private PlayerDto toDto(PlayerModel model)
        {
            ...
            //赋值好友列表
            string[] friends = model.FriendIdList.Split(',');
            dto.friends = new Frient[friends.Length];
            for (int i = 0; i < friends.Length; i++)
            {
                ...
                dto.friends[i] = new Frient(id, name, online); ;
            }
            return dto;
        }
